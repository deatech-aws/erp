...
$query ="SELECT c.cprogrammeid,COUNT(c.vmatricno) AS tot
      ...
      GROUP BY c.cprogrammeid, cfacultyid
      HAVING cfacultyID ='".$_SESSION['role_id']."'";

$query =" SELECT DISTINCT cprogrammeid
 FROM tbl_details
 WHERE vmatricno  IN (SELECT vmatricno FROM students  WHERE cfacultyID ='".$_SESSION['role_id']."' AND grad_approved =0)";
 // echo $query;
  $stmt = $conn->prepare($query, array(PDO::ATTR_CURSOR => PDO::CURSOR_FWDONLY));
  $stmt->execute();
  while ($rw = $stmt->fetch(PDO::FETCH_NUM, PDO::FETCH_ORI_NEXT))
...
<body>
    ...
    include "inc/f_sidebar.php";
    include "content/fac_dash_content.php";
