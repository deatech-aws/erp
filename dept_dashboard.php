...
$query ="SELECT departmentdesc
FROM department
WHERE cdepartmentid='".$_SESSION['dept_id']."'";
// var_dump($query );
// exit;
$fac_desc="";
...
$query ="SELECT c.cprogrammeid,COUNT(c.vmatricno) AS tot
      FROM cum_result_summary c INNER JOIN
      programme p ON p.cprogrammeid=c.cprogrammeid AND TCE >=imincredits
      GROUP BY c.cprogrammeid, cfacultyid
      HAVING cfacultyID ='".$_SESSION['fac_id']."'";

$query ="SELECT c.cprogrammeid,COUNT(c.vmatricno) AS tot
      FROM tbl_details c INNER JOIN
      students p ON p.cprogrammeid=c.cprogrammeid AND grad_approved=0
      GROUP BY c.cprogrammeid, cfacultyid
      HAVING cfacultyID ='".$_SESSION['fac_id']."'";

$query =" SELECT DISTINCT cprogrammeid
 FROM tbl_details
 WHERE vmatricno  IN (SELECT vmatricno 
 FROM students s inner join programme p ON p.cprogrammeid =s.cprogrammeid and  cdepartmentid ='".$_SESSION['dept_id']."' AND grad_approved =0)";
  // echo $query;
  // exit;
  $stmt = $conn->prepare($query, array(PDO::ATTR_CURSOR => PDO::CURSOR_FWDONLY));
  $stmt->execute();
...
<body>
    ...
    include "inc/d_sidebar.php";
    include "content/dept_dash_content.php";
...
var theme = {
  // color: [
  //           'green', 'blue', '#BDC3C7', 'yellow',
  //           'red', 'red', '#759c6a', '#9B59B6'
  //       ],
         color: [
...
var echartLine = echarts.init(document.getElementById('echart_line'), theme);
    ...
    echartLine.setOption({
      title: {
        text: "Faculty-Wise Performance(%) Distribution.",
        subtext:''
      },
      tooltip: {
        trigger: 'axis'
      ...
      legend: {
        x: 220,
        y: 40,
        data: ['Fail', '3rd Class', '2nd Class Lower', '2nd Class Upper', '1st Class']
      },
      toolbox: {
        show: true,
        feature: {
...
series: [{
  name: 'Fail',
        ...
        data: <?php echo json_encode($a_fail); ?>
        ...
        data: <?php echo json_encode($a_3rd); ?>
        ...
        data: <?php echo json_encode($a_2ndL); ?>
        ...
        data: <?php echo json_encode($a_2ndU); ?>
