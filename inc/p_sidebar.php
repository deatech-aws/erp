...
<!-- begin #sidebar -->
		...
		<div id="sidebar" class="sidebar" style="font-size:16px;color:white">
          ...
          <li class="has-sub">
						...
						<ul class="sub-menu " style="font-size: 16px!important;display: block;">
              <?php
              $llevel=0;
              $query="SELECT DISTINCT iYrlevel
                      FROM rst_class_of_degree
                      WHERE cprogrammeid='".$_SESSION['prog_id']."' AND batchno = '".$_SESSION['sess']."'
                      order by iYrlevel";
                      $stmt = $conn->prepare($query, array(PDO::ATTR_CURSOR => PDO::CURSOR_FWDONLY));
                      $stmt->execute();
            while ($rw = $stmt->fetch(PDO::FETCH_NUM, PDO::FETCH_ORI_NEXT)){
              $llevel=$rw[0];
              echo "
							<li><a href=\"lev_sess.php?Returnurl=".enc($rw[0])."\" target =\"_blank\">$rw[0] Level Result</a></li>

              ";
              }
              echo "
							<li><a  target = \"_blank\" href=\"prob_sess.php?Returnurl=".enc($_SESSION['prog_id'].'|'.$llevel)."\" >Probable Graduants</a></li>
