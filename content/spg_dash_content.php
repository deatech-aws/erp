...
$query="SELECT SUM(fail) AS fail,SUM(third_class) AS third_class,
  ...
  programme p ON p.cprogrammeid=r.cprogrammeid
  GROUP BY batchno,ceductgid
  HAVING ceductgid = '".$_SESSION["educat"]."' AND batchno = '".$_SESSION['sess']."'";
// echo $query;
  ...
  <div class="row">
    ...
    <!-- <div class="col-md-4"> -->
       <div class="pull-right" style="margin-top:-15px">
          <a href="pg_sess.php?Returnurl=<?php echo enc($_SESSION["educat"]); ?>" class="btn btn-xs btn-success" ><?php echo $_SESSION['dsc'];?></a>
        </div>
        <br>
      ...
      $query="SELECT c.cprogrammeid,vprogaward,
                ...
                SUM(first_class)AS first_class,clabel,ceductgid,iDuration
  							...
  							GROUP BY p.cprogrammeid,vprogaward,clabel,batchno,ceductgid,iDuration
                HAVING ceductgid ='".$_SESSION["educat"]."' AND  batchno = '".$_SESSION['sess']."'";
              // echo $query;
                   $stmt = $conn->prepare($query, array(PDO::ATTR_CURSOR => PDO::CURSOR_FWDONLY));
                   $stmt->execute();
                   ...
                   $str =implode(",",$prob_arr);
                     while ($rw = $stmt->fetch(PDO::FETCH_NUM, PDO::FETCH_ORI_NEXT))
                     {
                       ...
                       for ($i = 2;($i<=6);$i++){
                        ...
                        $p_btn="";
                      if ($pos!==false){
                        $p_btn ="<a class = \"viewresult btn btn-primary btn-xs\" target = \"_blank\" href=\"prob_sess.php?Returnurl=".enc($rw[0])."\" >Probation Graduant</a>";
                      }
