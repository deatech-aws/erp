...
$query="SELECT SUM(fail) AS fail,SUM(third_class) AS third_class,
  ...
  FROM rst_class_of_degree r inner JOIN
  programme d ON d.cprogrammeid = r.cProgrammeId
  GROUP BY cdepartmentid,batchno
  HaVING cdepartmentid='".$_SESSION['dept_id']."' AND batchno = '".$_SESSION['sess']."'";
//   var_dump($query);
//    exit; 
  ...
  <div class="row">
        <div class="widget-chart with-sidebar ">
            <div class="widget-chart-content" style="margin-top:-30px">
                <h3 class="text-black">
                    <?php echo $_SESSION['sess'];?> Class of Degree Analytics
    ...
    <!-- <div class="col-md-4"> -->
      <div class="panel panel-inverse" data-sortable-id="index-6">
        <div class="panel-heading">
          ...
          <h1 class="panel-title" style="font-size:14px"><?php echo $_SESSION['sess'];?> Departmental  Class of Degree Analytics Details</h1>
        </div>
      ...
      $query="SELECT c.cprogrammeid,vprogaward,
                ...
                SUM(first_class)AS first_class,clabel,ceductgid,iDuration
  							...
  							GROUP BY p.cprogrammeid,vprogaward,clabel,p.cdepartmentid,batchno,ceductgid,iDuration
                HAVING p.cdepartmentid='".$_SESSION['dept_id']."' AND batchno = '".$_SESSION['sess']."'
                ORDER BY vprogaward";
             // echo $query;
                   $stmt = $conn->prepare($query, array(PDO::ATTR_CURSOR => PDO::CURSOR_FWDONLY));
                   $stmt->execute();
                   ...
                   $str =implode(",",$prob_arr);
                     while ($rw = $stmt->fetch(PDO::FETCH_NUM, PDO::FETCH_ORI_NEXT))
                     {
                       ...
                       for ($i = 2;($i<=6);$i++){
                        ...
                        $p_btn="";
                        if ($pos!==false){
                          $p_btn ="<a class = \"viewresult btn btn-primary btn-xs\" target = \"_blank\" href=\"prob_sess.php?Returnurl=".enc($rw[0])."\" >Probable</a>";
                        }
                           ...
                           <div class = \"view_result\">
                             <a href=\"prog_sess.php?Returnurl=".enc($rw[0])."\" target =\"_blank\"><b> ".$rw[1]."</b></a>
                              ...
                              if($rw[8]=="PGX"){
                                ...
                                $i--;
                                echo  $p_btn;//"<a class = \"viewresult btn btn-primary btn-xs\" target = \"_blank\" href=\"prob_sess.php?Returnurl=".enc($rw[0].'|'.$i*100)."\" >Probation Graduant</a>";
                              }
                                 echo "
                               </div>
                            </div>
