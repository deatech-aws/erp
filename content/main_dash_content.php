...
// echo $query;
  ...
  while ($rw = $stmt->fetch(PDO::FETCH_NUM, PDO::FETCH_ORI_NEXT))
  {
    ...
    $json_array['value']=number_format($rw[0],"0");
    array_push($json_data,$json_array);
    $json_array['label']='Third Class';
    $json_array['value']=number_format($rw[1],"0");
    array_push($json_data,$json_array);
    $json_array['label']='2.2';
    $json_array['value']=number_format($rw[2],"0");
    array_push($json_data,$json_array);
    $json_array['label']='2.1';
    $json_array['value']=number_format($rw[3],"0");
    array_push($json_data,$json_array);
    $json_array['label']='First Class';
    $json_array['value']=number_format($rw[4],"0");
    array_push($json_data,$json_array);
  }
...
<div id="content" class="content" style="margin-top:30px">
  ...
  <!-- end breadcrumb -->
  <!-- begin page-header -->
  <h1 class="page-header"><?php echo 'Faculty of '. $fac_desc.' '. $_SESSION['sess']; ?> Semester Overall Result Analytics Dashboard <small>Academic Performance Distribution...</small></h1>
  <!-- end page-header -->
  ...
  <div class="row">
      ...
      <div class="panel panel-inverse" data-sortable-id="index-6">
        <div class="panel-heading">
          ...
          <h4 class="panel-title"><?php echo $_SESSION['sess'];?> Class of Degree Distribution By Faculties</h4>
        </div>
          ...
          <table id="tbl_zone" class="table table-striped" >
            <thead>
              <tr style="font-size:14px">
                <th style="font-size:18px">Faculty</th>
                           ...
                           while ($rw = $stmt->fetch(PDO::FETCH_NUM, PDO::FETCH_ORI_NEXT))
                           {
                             ...
                             for ($i = 2;($i<=6);$i++){
                              ...
                              echo "<tr style=\"font-size:16px\">
                                 ...
                                 <td style=\"font-size:16px\">".number_format(($rw[2]/$rtotal)*100,"2")."% (".number_format($rw[2],"0").")</td>
                                 <td style=\"font-size:16px\">".number_format(($rw[3]/$rtotal)*100,"2")."% (".number_format($rw[3],"0").")</td>
                                 <td style=\"font-size:16px\">".number_format(($rw[4]/$rtotal)*100,"2")."% (".number_format($rw[4],"0").")</td>
                                 <td style=\"font-size:16px\">".number_format(($rw[5]/$rtotal)*100,"2")."% (".number_format($rw[5],"0").")</td>
                                 <td style=\"font-size:16px\">".number_format(($rw[6]/$rtotal)*100,"2")."% (".number_format($rw[6],"0").")</td>
        ...
        <div class="widget-chart with-sidebar ">
            <div class="widget-chart-content" style="margin-top:-30px">
                <h3 class="text-black">
                    Class of Degree Distribution Chart
                    <small>Performace Standing Report</small>
                </h3>
