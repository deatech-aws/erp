...
class Students{   
	...
	function read(){	
		if($this->id) {
		$sql ="SELECT vmatricno AS id,concat(vlastname,' ',vothernames) AS vname,vfacultydesc AS faculty,p.vprogaward AS program,dateofbirth AS dob,cgender AS sex,p.ceductgid AS eduCtg,f.cfacultyid
				FROM students s INNER JOIN
				faculty f ON f.cfacultyid=s.cfacultyid inner join
				programme p ON p.cprogrammeid=s.cprogrammeid
				AND vmatricno ='".$this->id."'";

		$sql ="SELECT vmatricno AS id,vname,vfacultydesc AS faculty,vprogaward AS program,vProgrammeDesc as specialization,dateofbirth AS dob,cgender AS sex,ceductgid AS eduCtg,cfacultyid
			FROM archive_stud
	...
	$sql="SELECT r.coursecode AS code,a.coursetitle AS title,grade,r.creditunits AS unit,points,gpoints,
			CASE  
			WHEN grade='A' THEN 'Excellent'
			WHEN grade='B' THEN 'Very Good'
			WHEN grade='C' THEN 'Good' 
